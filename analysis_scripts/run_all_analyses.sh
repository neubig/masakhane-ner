...
for i in $(seq 0 1); do
  ...
  if [[ -e $masadir/entity_analysis/XLM-R/${mycode}_xlmr_test_predictions.txt ]]; then
    python3 add_gold.py $masadir/entity_analysis/XLM-R/${mycode}_xlmr_test_predictions.txt $masadir/data/$mycode/test.txt > $iedir/results/${mycode}_xlmr_test.txt
  else
  ...
  if [[ -e $masadir/entity_analysis/biLSTM_CRF/test.news_${myname}_model ]]; then
    cp $masadir/entity_analysis/biLSTM_CRF/test.news_${myname}_model $iedir/results/${mycode}_bilstmcrf_test.txt
    cp $masadir/entity_analysis/binary_GAZ/test.news_${myname}_model_bi $iedir/results/${mycode}_binarygaz_test.txt
  else
    echo "WARNING: $masadir/entity_analysis/biLSTM_CRF/test.news_${myname}_model doesn't exist"
  fi
done
...
for i in $(seq 0 1); do
  mycode=${lang_codes[$i]}
  bash ../../run_task_ner_masakhane.sh $mycode
done
